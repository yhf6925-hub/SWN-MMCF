...
For an NVIDIA GPU environment, install the PyTorch CUDA wheel generated by the official selector, then install the remaining packages:

```bash
# Run the PyTorch CUDA installation command generated for this machine:
# https://pytorch.org/get-started/locally/

python -m pip install numpy onnx onnxscript onnxruntime
```

`onnxruntime` is sufficient for validating this exported MLP on CPU. Install `onnxruntime-gpu` instead only when Python-side ONNX validation also needs CUDA; do not install both packages in the same virtual environment.
...
print("Python/PyTorch environment: OK")
...
Training data are converted to `float32` and the aligned arrays are retained in memory by the dataset loader. Use an SSD and provide RAM larger than the uncompressed `.npz` contents. GPU memory usage is mainly determined by the batch size and the first dense layer; reduce the batch size when CUDA memory is insufficient. Because the encoder uses batch normalization, the configured batch size must be greater than one. The training entry point is `train.py`, while `model.py` contains feature normalization, the encoder, the training-only decoder, and the ONNX deployment graph.
